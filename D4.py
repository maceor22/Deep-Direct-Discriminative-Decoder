...
class DeepDirectDiscriminativeDecoder1(nn.Module):

    def __init__(
            self, prediction_model, transition_model, latent_dim,
            proposal_covariance_type = 'diag', proposal_variance_init = None,
            penalty_model = None, epsilon = 1e-20,
    ):
        super(DeepDirectDiscriminativeDecoder1, self).__init__()
        ...
        if proposal_covariance_type == 'diag':
            if proposal_variance_init is not None:
                Q_tril_components = (proposal_variance_init**0.5).log()
            else:
                Q_tril_components = torch.zeros((latent_dim,)).uniform_(-8,-4)

        elif proposal_covariance_type == 'full':
            Q_tril_components = torch.zeros((int((latent_dim * (latent_dim+1))/2),))
            diag_ix = torch.arange(1, latent_dim+1).cumsum(0).long() - 1
        ...
        self.Q_tril_components = nn.Parameter(
            Q_tril_components,#.to(device), 
            requires_grad = True)


        self.latent_dim = latent_dim
        self.proposal_covariance_type = proposal_covariance_type
        self.epsilon = epsilon
        #self.device = device
    ...
    def _proposal(self, x_prev):
        ...
        if self.proposal_covariance_type == 'diag':
            for i in range(self.latent_dim):
                Q_cholesky_tril[i,i] = self.Q_tril_components[i].exp()

        elif self.proposal_covariance_type == 'full':
            idx = 0
            for i in range(self.latent_dim):
                for j in range(i+1):
                    if i == j:
    ...
    def _denominator(self, x_prev, x_curr):        
        state = self._state(
            x_prev.expand(x_curr.size(0), x_prev.size(0), -1),
            x_curr.expand(x_curr.size(0), x_prev.size(0), -1),
        )
        log_prob = torch.logsumexp(state, dim = -1)
        return log_prob
    

    def _state(self, x_prev, x_curr):
        state = self.transition.log_prob(x_prev.unsqueeze(-2), x_curr)
        state = state - self._proposal(x_prev).log_prob(x_curr)
        return state
    ...
    def importance_resample(self, yt, unweighted_particles, particle_trace):
        ...
        # combine observation prob with in-maze prob of unweighted particles
        quality = self.penalty(unweighted_particles.to('cpu')).to(yt.device)
        #print('\n In-maze count: ', quality.sum())
    ...
    def filter(self, observations, n_particles = 1, smoother = False):
        ...
        particles_init = torch.distributions.Uniform(0,1).sample(
            (n_particles, self.latent_dim)).to(observations.device)
        unweighted_particles = self._proposal(particles_init).sample()
    ...
    def log_prob(
        ...
        if component in ['numerator', 'ratio', 'full']:
            numer = self.prediction.log_prob(observations, latent)[1:]
        
        if component in ['denominator', 'ratio', 'full']:
            filter_trace = self.filter(observations, n_particles).to(observations.device).detach()

            denom = torch.logsumexp(
                self._state(filter_trace.transpose(0,1)[:-1], latent[1:].expand(n_particles,-1,-1).transpose(0,1)), 
                dim = -1,
                )
            denom = denom - torch.tensor(
                n_particles, dtype = denom.dtype, device = denom.device, requires_grad = denom.requires_grad,
...
class DeepDirectDiscriminativeDecoder2(nn.Module):

    def __init__(
            self, latent_dim, prediction_model, 
            transition_model, proposal_model = None,
            penalty_model = None, epsilon = 1e-20,
    ):
        super(DeepDirectDiscriminativeDecoder2, self).__init__()
    ...
    def _step(self, x_prev):
        if self.proposal != None:
            return self.proposal(x_prev).sample()
        else:
            return self.transition(x_prev).sample()
        

    def _state(self, x_prev, x_curr):
        state = self.transition.log_prob(x_prev, x_curr)
        
        if self.proposal != None:
            state = state - self.proposal.log_prob(x_prev, x_curr)
        return state
    

    def _denominator(self, x_prev, x_curr):        
        state = self._state(
            x_prev.expand(x_curr.size(0), x_prev.size(0), -1).unsqueeze(-2),
            x_curr.expand(x_curr.size(0), x_prev.size(0), -1),
        )
    ...
    def importance_resample(self, yt, unweighted_particles, particle_trace):
        ...
        if self.proposal != None:
            state = self._state(particle_trace[:,-1:,:], unweighted_particles)
            prob = (state + numer - denom).exp() * quality + self.epsilon
        else:
            prob = (numer - denom).exp() * quality + self.epsilon
    ...
    def forward(self, yt, particle_trace):
        # obtain target distribution of next particles and sample
        unweighted_particles = self._step(particle_trace[:,-1:,:])
    ...
    def filter(self, observations, n_particles = 1, smoother = False):
        ...
        particles_init = torch.distributions.Uniform(0,1).sample(
            (n_particles, self.latent_dim)).to(observations.device)
        unweighted_particles = self.transition.predict(particles_init.unsqueeze(1))
        ...
        if smoother:
            return torch.stack(self.w_particles, dim = 1).to(observations.device), particle_trace
        
        else:
            return torch.stack(self.w_particles, dim = 1).to(observations.device)
    ...
    def log_prob(
        ...
        if component in ['denominator', 'ratio', 'full']:
            ...
            denom = torch.logsumexp(
                self.transition.log_prob(
                    filter_trace.transpose(0,1)[:-1].unsqueeze(-2), 
                    latent[1:].expand(n_particles,-1,-1).transpose(0,1)), 
                dim = -1,
                )
            denom = denom - torch.tensor(
                n_particles, dtype = denom.dtype, device = denom.device, requires_grad = denom.requires_grad,
        ...
        if component in ['state', 'full']:
            state = self.transition.log_prob(latent[:-1].unsqueeze(-2), latent[1:])
