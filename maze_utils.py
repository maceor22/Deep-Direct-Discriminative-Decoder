...
# =============================================================================
    ...
    def generate_data(
            self, input_history_length = 0, spike_bin_size = 1, 
            label_history_length = 0, shuffle = False,
        ):
        lower_index = max(
            0 if input_history_length == 0 else input_history_length+1,
            label_history_length,
            )
        
        if input_history_length == 0:
            spike_data = self.spike_counts[lower_index:-1,:].unsqueeze(1)
            pos_data = self.pos[lower_index:-1,:].unsqueeze(1)
            
        else:
            spike_hist = torch.cat([
                self.spike_counts[i-(input_history_length+1):i,:].unsqueeze(0) 
                for i in range(lower_index, self.spike_counts.size(0))
                ], dim = 0)
            
            pos_data = torch.cat([
                self.pos[i-(input_history_length+1):i,:].unsqueeze(0) 
            ...
            if spike_bin_size == 1:
                ...
                num_bins = int(input_history_length/spike_bin_size)
                spike_data = []
        ...
        if label_history_length == 0 and input_history_length == 0:
            label_data = self.pos[lower_index+1:,:]
        elif label_history_length == 0:
